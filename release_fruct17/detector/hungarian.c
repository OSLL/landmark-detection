...
#define hungarian_test_alloc(X) do {if ((void *)(X) == NULL) fprintf(stdout, "Out of memory in %s, (%s, line %d).\n", __FUNCTION__, __FILE__, __LINE__); } while (0)
...
int hungarian_init(hungarian_problem_t* p, int** cost_matrix, int rows, int cols, int mode) {
  ...
  if (mode == HUNGARIAN_MODE_MAXIMIZE_UTIL) {
    ...
    fprintf(stdout,"%s: unknown mode. Mode was set to HUNGARIAN_MODE_MINIMIZE_COST !\n", __FUNCTION__);
...
void hungarian_solve(hungarian_problem_t* p)
{
  ...
  if (verbose)
    fprintf(stdout, "Using heuristic\n");
  for (l=0;l<n;l++)
    {
      s=p->cost[0][l];
      for (k=1;k<m;k++) 
	...
	if (s==p->cost[k][l] && row_mate[l]<0)
	  {
	    ...
	    row_mate[l]=k;
	    if (verbose)
          fprintf(stdout, "matching col %d==row %d\n",l,k);
	    goto row_done;
	  }
      col_mate[k]= -1;
      if (verbose)
    fprintf(stdout, "node %d: unmatched row %d\n",t,k);
      unchosen_row[t++]=k;
    row_done:
      ;
    }
  ...
  while (1)
		      ...
		      if (del<slack[l])
			{
			  if (del==0)
				...
				goto breakthru;
			      ...
			      parent_row[l]=k;
			      if (verbose)
                fprintf(stdout, "node %d: row %d==col %d--row %d\n",
				       t,row_mate[l],l,k);
    ...
    breakthru:
      ...
      if (verbose)
    fprintf(stdout, "Breakthrough at node %d of %d!\n",q,t);
      while (1)
	{
	  j=col_mate[k];
	  col_mate[k]=l;
	  row_mate[l]=k;
	  if (verbose)
        fprintf(stdout, "rematching col %d==row %d\n",l,k);
	  if (j<0)
	    break;
	  k=parent_row[j];
	  l=j;
